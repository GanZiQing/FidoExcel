...
//namespace ExcelAddIn2.Excel_Pane_Folder
{
    class CommonUtilities
    {
        ...
        #region Terminate Excel Range at Null
        public static void TerminateRangeAtFirstNullCell(ref Range range)
        {
            // This assumes check is done based on first column, probably should update to include other columns as well 
            (int startRow, int endRow, int startCol, int endCol) = GetRangeDetails(range);
            Worksheet ws = range.Worksheet;
            int finalRowNum = -1;
            for (int rowNum = startRow; rowNum <= endRow; rowNum++)
            {
                Range cell = ws.Cells[rowNum, startCol];
            ...
            else
            {
                ...
                Range endCell = ws.Cells[finalRowNum, startCol];
