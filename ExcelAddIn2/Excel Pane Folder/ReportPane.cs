...
namespace ExcelAddIn2.Excel_Pane_Folder
{
    public partial class ReportPane : UserControl
    {
        ...
        {
            ReportPaneBackup reportPaneBackup = new ReportPaneBackup();            
            tabControl1.TabPages.Add(reportPaneBackup.GetPageTaskPane());
        }
        #endregion
        ...
        private void importToPpt_Click(object sender, EventArgs e)
        {
            ...
            for (int rowNum = 0; rowNum < imagePaths.Length; rowNum++)
            {
                ...
                {
                    MessageBox.Show($"Text Box named 'Description' not found, please create it in the template and name it using selection pane.\n\n" +
                        $"{ex.Message}" +
